...
## GitHub 이슈 연동 규칙
- **스켈레톤 생성 시**: 파일 생성과 함께 GitHub 이슈도 생성
  - 제목: `[백준 {번호}] {제목}` 또는 `[LeetCode {번호}] {제목}`
  - 라벨: 알고리즘 유형(`DFS`, `BFS`, `DP` 등), 난이도(`실버`, `골드` 등), 플랫폼(`백준`, `LeetCode`)
  - 본문: 난이도, 알고리즘, 파일명, 문제 링크 포함
- **풀이 완료 시**: 파일 상태를 COM으로 변경하고, 해당 이슈도 close 처리
  - 커밋 메시지에 `closes #{이슈번호}` 포함하면 자동 close

## 스켈레톤 파일 생성 규칙
- 문제 풀이용 빈 파일을 만들 때, 아래 내용을 주석에 포함할 것:
  - 문제 설명, 입력, 출력은 **백준 원문 그대로** 복사 (풀어쓰지 않음)
  - 예제 입력/출력 (최소 1개)
  - 시간제한, 메모리제한
- 문제를 보고 "그래프인지 그리드인지" 스스로 판단하는 것도 연습이므로, 힌트성 설명을 추가하지 않음
